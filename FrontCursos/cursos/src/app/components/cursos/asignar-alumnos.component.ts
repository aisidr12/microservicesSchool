...
export class AsignarAlumnosComponent implements OnInit {
  ...
  mostrarColumnas: string[] = ['nombre', 'apellido', 'seleccion']  // Nombres de las columnas definidas como si fuera su Id
  alumnos: Alumno [] = [];
  ...
  filtrar(nombre: string): void {
    ...
    if (nombre !== '') {
      //Realizamos la llamada al backend para el filtrado por nombres dentro de un curso
      this.alumnoService.filtarPorNombre(nombre).
        subscribe(alumno => {
          //Dentro de los alumnos que están filtramos a esos alumnos que esten dentro del curso
          this.alumnosAsignar = alumno.filter(a => {
            let filtrar = true;
      ...
      this.seleccion.clear();
        ...
        if(e.status ===500){
          ...
          if(mensaje.indexOf('ConstraintViolationException')> -1){
            Swal.fire("Cuidado:", `No se puede asignar al alumno ya asociado a otro curso.`,'error');
          }
        }
    });
  }
  ...
  eliminarAlumno(alumno:Alumno):void{
    ...
    if(result.value){
      this.cursoService.eliminarAlumno(this.curso,alumno).subscribe(curso =>{
        ...
        this.iniciarPaginador();
        Swal.fire("Eliminado:", `Alumno ${alumno.nombre} elimnado del curso.`,'success');
